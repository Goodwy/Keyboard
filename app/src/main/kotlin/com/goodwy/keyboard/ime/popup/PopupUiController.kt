...
import com.goodwy.keyboard.ime.keyboard.DefaultComputingEvaluator
...
class PopupUiController(
    ...
    fun RenderPopups(): Unit = with(LocalDensity.current) {
        baseRenderInfo?.let { renderInfo ->
            PopupBaseBox(
                modifier = Modifier
                    .requiredSize(Size(renderInfo.bounds.size.width, renderInfo.bounds.size.height * 0.4f).toDpSize())//renderInfo.bounds.size.toDpSize() //TODO Popup size
