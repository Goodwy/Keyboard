...
import com.goodwy.keyboard.ime.editor.InputAttributes
import com.goodwy.keyboard.ime.input.CapitalizationBehavior
import com.goodwy.keyboard.ime.input.InputEventDispatcher
import com.goodwy.keyboard.ime.input.InputKeyEventReceiver
import com.goodwy.keyboard.ime.input.InputShiftState
import com.goodwy.keyboard.ime.nlp.ClipboardSuggestionCandidate
...
import com.goodwy.keyboard.lib.uppercase
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
    ...
    init {
        scope.launch(Dispatchers.Main.immediate) {
            ...
            prefs.keyboard.dotKeyEnabled.observeForever {
                updateActiveEvaluators()
            }
            prefs.keyboard.bigEnterButton.observeForever {
                updateActiveEvaluators()
            }
            prefs.gestures.hideAllLabel.observeForever {
    ...
    private fun handleLanguageSwitch() {
        when (prefs.keyboard.utilityKeyAction.get()) {
            ...
            UtilityKeyAction.SWITCH_LANGUAGE -> switchToNextSubtype()//subtypeManager.switchToNextSubtype()
    ...
    private fun switchToNextSubtype() {
        if (prefs.localization.switchEmojiWhenChangingLanguage.get()) {
            if (subtypeManager.activeSubtype == subtypeManager.subtypes.lastOrNull()) activeState.imeUiMode = ImeUiMode.MEDIA
        }
    ...
    private fun switchToPrevSubtype() {
        if (prefs.localization.switchEmojiWhenChangingLanguage.get()) {
            if (subtypeManager.activeSubtype == subtypeManager.subtypes.firstOrNull()) activeState.imeUiMode = ImeUiMode.MEDIA
        }
    ...
    override fun onInputKeyUp(data: KeyData) = activeState.batchEdit {
        when (data.code) {
            ...
            KeyCode.IME_PREV_SUBTYPE -> switchToPrevSubtype() //subtypeManager.switchToPrevSubtype()
            KeyCode.IME_NEXT_SUBTYPE -> switchToNextSubtype() //subtypeManager.switchToNextSubtype()
    ...
    private inner class ComputingEvaluatorImpl(
        ...
        override fun evaluateVisible(data: KeyData): Boolean {
            ...
            return when (data.code) {
                ...
                KeyCode.PHONE_PAUSE -> !(mode == KeyboardMode.CHARACTERS && !prefs.keyboard.commaKeyEnabled.get())
                KeyCode.DOT -> !(mode == KeyboardMode.CHARACTERS && !prefs.keyboard.dotKeyEnabled.get())
                else -> true
            }
        }
